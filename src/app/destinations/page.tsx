...
export const metadata: Metadata = {
  ...
  description:
    "Nepal regions beyond a single trail — treks, cities, and safaris on TrailSathi.",
};
...
const destinations = [
  ...
  },
  {
    ...
    href: "/community",
    image:
      "https://images.unsplash.com/photo-1605647540926-0962900559c9?w=1200&q=80",
    comingSoon: true,
  },
];
    ...
    <main className="min-h-screen bg-stone-50">
      ...
      <section className="relative overflow-hidden bg-white">
        <div
          ...
          style={{
            backgroundImage:
              "radial-gradient(circle at 80% 30%, #047857 1.5px, transparent 1.5px)",
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-16">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-4 text-4xl font-bold tracking-tight text-stone-900 sm:text-5xl">
              Nepal beyond a single trail
            </h1>
            <p className="mt-4 max-w-2xl text-lg leading-8 text-stone-600">
              Trekking is the core of TrailSathi today — verified routes, permits, and guides.
              These destination hubs will grow into full guides for city breaks, safaris, and
              multi-region trips.
            </p>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-20 pt-10">
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {destinations.map((d) => (
            <article
              key={d.slug}
              className="group overflow-hidden rounded-2xl border border-stone-200 bg-white transition-all hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-lg"
            >
              <Link href={d.href} className="block">
                <div className="relative h-48 w-full overflow-hidden">
                  ...
                  <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between gap-2">
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-white/95 px-3 py-1 text-xs font-semibold text-stone-800 shadow-sm">
                      <MapPin size={12} aria-hidden />
                      {d.comingSoon ? "Expanding" : "Explore"}
                    </span>
                    {d.comingSoon ? (
                      <span className="rounded-full bg-amber-500/95 px-2.5 py-0.5 text-xs font-semibold text-white shadow-sm">
                        Coming soon
                      </span>
                    ) : null}
                  </div>
